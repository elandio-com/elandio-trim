...
const pathModeOn = document.getElementById('path-mode-on');
const pathModeOff = document.getElementById('path-mode-off');
...
const pathModeRadios = document.querySelectorAll('input[name="path_mode"]');
if (pathWarningModal && pathModeRadios.length > 0) {
...
if (cancelPathChangeBtn && pathWarningModal) {
...
if (confirmPathChangeBtn && pathWarningModal) {
...
function togglePathModeUI(isRoot) {
    if (!pathModeOn || !pathModeOff) return;
    if (isRoot) {
...
function renderTableRows(links) {
    ...
    linksList.innerHTML = links.map(link => {
        ...
        const shortPath = `/${safeSlug}`.replace('//', '/');
        const shortUrl = `${window.location.host}/${safeSlug}`;
        return `
        <tr class="bg-white border-b border-slate-50 hover:bg-slate-50/50 transition-colors">
            <td class="px-6 py-4 font-medium text-brand-600">
                <a href="/${safeSlug}" target="_blank" class="flex items-center hover:underline">
                    /${safeSlug}
                </a>
            </td>
            <td class="px-6 py-4">
                <div class="text-slate-500 truncate max-w-xs" title="${safeTargetUrl}">${safeTargetUrl}</div>
            </td>
            ...
            <td class="px-6 py-4 text-right">
                <div class="flex items-center justify-end space-x-2">
                    ...
                    <button class="action-btn-edit p-1.5 text-slate-400 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors" title="Edit" data-slug="${safeSlug}" data-url="${safeTargetUrl}">
                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    </button>
                    <button class="action-btn-delete p-1.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors" title="Delete" data-slug="${safeSlug}">
